...
const PropertyGrid = ({ properties, isAdmin = true, onEdit, onDelete }) => {
  // Get formatPrice and formatPriceRange from store
  const { formatPrice, formatPriceRange } = usePropertyStore();
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {properties.map((property) => {
        // Parse the price - handle both direct price and starting_price
        const price = property.price || property.starting_price;
        const priceValue = typeof price === 'string' ? parseFloat(price) : price;
        
        // Format the price using store function
        const formattedPrice = priceValue ? formatPrice(priceValue) : "POA";
        ...
        return (
          ...
          >
            ...
            {property.property_images?.length > 0 ? (
              ...
              <div className="w-full h-[204px] bg-gray-200 flex items-center justify-center text-gray-500">
                No Image Available
              </div>
            )}
